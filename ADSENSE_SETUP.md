...
2. AdSense でレスポンシブのディスプレイ広告ユニットを 1 つ作成する。
3. Cloudflare Dashboard の Workers & Pages で、このサイトの build environment variables に以下を追加する。

```text
PUBLIC_ADSENSE_CLIENT=ca-pub-発行されたパブリッシャーID
PUBLIC_ADSENSE_SLOT_ARTICLE_BOTTOM=作成した広告ユニットのslot番号
```

4. 再デプロイ後、記事末尾に `広告` 表記つきの枠が表示されることを確認する。
